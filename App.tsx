...
const App: React.FC = () => {
  ...
  const getResponse = async (text: string, userMsg: Message, media?: MediaData) => {
    ...
    try {
      const stream = await generateAssistantStream(text, historySnapshot.slice(0, -1), statsString, selectedGoal || undefined, media);
      let fullContent = "";
      for await (const chunk of stream) {
        fullContent += chunk.text || "";
        setMessages(prev => prev.map(m => m.id === assistantMsgId ? { ...m, content: fullContent.split("---")[0].trim() } : m));
      }
      ...
      setMessages(prev => prev.map(m => m.id === assistantMsgId ? { 
        ...m, content: fullContent.split("---")[0].trim(), stats, quizData, intentData 
      } : m));

    } catch (error: any) {
      console.error("Stream Error:", error);
      setMessages(prev => prev.map(m => m.id === assistantMsgId && m.content === '' ? { ...m, content: "Synthesis layer interrupted. Please check your connectivity and try again." } : m));
    } finally {
      setIsTyping(false);
    }
  };
      ...
      <main ref={scrollRef} className={`flex-1 flex flex-col ${appState === AppState.INITIAL ? 'overflow-hidden justify-center' : 'overflow-y-auto justify-start'} p-6 md:px-12 space-y-2 scrollbar-hide`}>
        ...
        {appState === AppState.CHATTING && (
          <div className="max-w-4xl mx-auto w-full space-y-6 pt-6 pb-48">
            ...
            {isTyping && <div className="flex justify-start py-8 px-10 opacity-30 animate-pulse">...</div>}
          </div>
        )}
      </main>
          ...
          <div className="max-w-4xl mx-auto pointer-events-auto">
            <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(inputValue); }} className="relative flex flex-col gap-3">
              <div className={`relative glass bg-white/70 rounded-full p-1.5 flex items-center gap-1 border transition-all duration-500 shadow-2xl ${isQuizPending ? 'bg-slate-50/50 grayscale opacity-70 cursor-not-allowed' : 'border-slate-200 hover:border-slate-300'}`}>
                <button type="button" onClick={() => fileInputRef.current?.click()} className="w-11 h-11 flex items-center justify-center rounded-full text-slate-400 hover:text-indigo-600 hover:bg-indigo-50/50 transition-colors">
                ...
                />
                <button type="submit" disabled={(!inputValue.trim() && !selectedMedia) || isTyping || isQuizPending} className={`h-11 w-11 rounded-full flex items-center justify-center transition-all ${isQuizPending ? 'bg-slate-100 text-slate-300' : 'bg-slate-900 text-white shadow-lg'}`}>
